...
class Tokenizer:
    ...
    def __init__(
        ...
        user_dict: str = os.path.join(
            os.path.split(os.path.realpath(__file__))[0], "user_dict.txt"),
        ...
        self.thul = None
        if granularity == "word":
            self.thul = thulac.thulac(user_dict=user_dict)
    ...
    def __call__(self, input_strings: List[str]) -> List:
        """
        分词函数
    ...
    def split_char(self, input_strings: List[str]) -> List:
        ...
        for input_string in input_strings:
            ...
            segment_string = segment_string.replace(
                "[ 缺 失 成 分 ]", "[缺失成分]").split(" ")  # 缺失成分当成一个单独的token
            results.append([(char, "unk",
                             pinyin(char, style=Style.NORMAL,
                                    heteronym=True)[0])
    ...
    def split_word(self, input_strings: List[str]) -> List:
        ...
        """
        if self.segmented:
            seged_list = self.thul.cut(
                [input_string.split(" ") for input_string in input_strings])
        else:
            result = []
            for input_string in input_strings:
                seged_list = self.thul.cut(input_string)
                temp = []
                for (s, p) in seged_list:
                    pinyin = [lazy_pinyin(word)[0] for word in s]
                    temp.append((s, p, pinyin))
                result.append(temp)
        return result
